...
public class CombatController {
	...
	/* TODO: NPCs (Mobs) -> zufällige Anzahl 1-4 bei Öffnen der View
	 * 1. Zufallszahl zwischen 1 und 4 festlegen für Anzahl der Gegner
	 * 2. Zufallszahl (derzeit!) zwischen 1-9 für zufällige Gegnerauswahl
	 * 3. -> SQL-Methode: getAllMobs() -> für alle NPCs mit Category="Mob"
	 * 4. Mobs in Arrayliste laden für spätere Zufallsauswahl
	 * 5. NPCModels je nach Anzahl erstellen -> Methode
	 * 
	 * 6. Methoden für den Kampf 
	 * calculateRound
	 * calculateDamage -> HP reduzieren, prüfen ob HP=0
	 * checkConditions (win (zurück zur Map -> XP, Gold, Items... hinzufügen) or lose(zurück zur Taverne -> Spieler wird geheilt)
	 * healCharacter (verwenden von Tränken, bei Besuch der Taverne, auf der Map...)
	...
	private void processPlayerAction(String action, NpcModel target) {
		switch (action) {
		case "Angreifen": {
			playerAttack(target);
			break;
	...
	public List<NpcModel> setEnemiesForCombat(int numberOfEnemies) {
		allNpcsList = sqlController.getAllNpcs(); // Method to retrieve all possible NPCs from DB,
												  // Return by SQL: List<NPCModel>
	...
	public boolean isCombatRunning(List<NpcModel> combatantsNpcList) {
		if (this.character.getCurrentHP() <= 0) {
	...
	public int createRngForEnemySelection() {
		return new Random().nextInt(9) + 1; //TODO: sqlController.getAllMobs.getLength() instead of "9"
	}
